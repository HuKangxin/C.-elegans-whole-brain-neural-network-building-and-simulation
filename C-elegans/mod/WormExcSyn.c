...
#define nrn_init _nrn_init__WormExcSyn
#define _nrn_initial _nrn_initial__WormExcSyn
#define nrn_cur _nrn_cur__WormExcSyn
#define _nrn_current _nrn_current__WormExcSyn
#define nrn_jacob _nrn_jacob__WormExcSyn
#define nrn_state _nrn_state__WormExcSyn
#define _net_receive _net_receive__WormExcSyn 
#define state state__WormExcSyn 
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 1;
 	...
 	ivoc_help("help ?1 WormExcSyn D:/download/yanjjiusheng/hukangxin/hukangxin/mod/WormExcSyn.mod\n");
...
/*CVODE*/
 static int _ode_spec1 () {_reset=0;
 {
   if ( k  == 0.0 ) {
     ...
     Ds = 3.0 * ( 1.0 - s ) / ( 1.0 + exp ( - 0.1 * ( Vpre - 95.0 ) ) ) - 0.1 * s ;
     }
   }
 return _reset;
}
 ...
 if ( k  == 0.0 ) {
   ...
   Ds = Ds  / (1. - dt*( ( ( 3.0 )*( ( ( - 1.0 ) ) ) ) / ( 1.0 + exp ( - 0.1 * ( Vpre - 95.0 ) ) ) - ( 0.1 )*( 1.0 ) )) ;
   }
  return 0;
}
 /*END CVODE*/
 static int state () {_reset=0;
 {
   if ( k  == 0.0 ) {
      ...
      s = s + (1. - exp(dt*(( ( 3.0 )*( ( ( - 1.0 ) ) ) ) / ( 1.0 + exp ( - 0.1 * ( Vpre - 95.0 ) ) ) - ( 0.1 )*( 1.0 ))))*(- ( ( ( 3.0 )*( ( 1.0 ) ) ) / ( 1.0 + exp ( - 0.1 * ( Vpre - 95.0 ) ) ) ) / ( ( ( 3.0 )*( ( ( - 1.0 ) ) ) ) / ( 1.0 + exp ( - 0.1 * ( Vpre - 95.0 ) ) ) - ( 0.1 )*( 1.0 ) ) - s) ;
     }
   }
  return 0;
}
...
{
 { error =  state();
 if(error){fprintf(stderr,"at line 38 in file WormExcSyn.mod:\n	SOLVE state METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 }}}
...
#if NMODL_TEXT
static const char* nmodl_filename = "WormExcSyn.mod";
static const char* nmodl_file_text = 
  "NEURON {\n"
